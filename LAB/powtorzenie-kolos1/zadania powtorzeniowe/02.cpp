/*
    ...
    których wartości znajdują się w tablicy arr2.
    Jeżeli w arr1 nie ma odpowiedniego indeksu to wyświetla dla niego wartość NAN.
...
void f(float arr1[], int n, int arr2[], int m) {
    ...
    {
        if (arr2[i] < n) {
            printf("[%d]=%.2f ", arr2[i], arr1[arr2[i]]);
        }
        else {
            printf("[%d]=NAN ", arr2[i]);
        }
